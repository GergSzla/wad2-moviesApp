...
describe("Discover TV Shows Page", () => {
    ...
    describe("Add TV Shows To Favs", () => {
        it("Should add TV Shows to favourites", () => {
    ...
    describe("TV Shows In Favs", () => {
        it("Should Show TV Shows In Favs", () => {
            cy.get("nav").find(".favs_dropdown").click();
            cy.get("nav").find("#fav_tvs").click();
            cy.get("h1").contains("Favorite TV Shows");
            const searchString = 'u'
            const matchingTvs = filterByTitle(tvs, searchString);
            cy.get(".card").should("have.length", matchingTvs.length);
            cy.get(".card").each(($card, index) => {
